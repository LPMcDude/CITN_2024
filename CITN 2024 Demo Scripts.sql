/*
    CITN - SQL Server Tips and Tricks
...
Remember: Read Right to Left and Bottom to Top
...
--Turn off Query Plan before running this:
...
;WITH groupattendance AS (
    ...
    WHERE a.DidAttend = 1
    AND a.StartDateTime >= '2024-09-01'
    --ORDER BY g.name
)
SELECT p.firstname, gt.GroupName, gt.StartDateTime, TotalGroupAttendences
FROM Person p 
INNER JOIN groupattendance gt ON gt.PersonId = p.id AND gt.Rowid = 1 --Don't forget Inner Join is a type of Filter!
WHERE p.createddatetime >= '2024-06-07'
...
*/
SELECT so.object_id, sc.name AS SchemaName, so.name AS TableName, so.create_date, so.modify_date, sp.TotalRows, si.index_id, si.Name AS IndexName
...
--> But what if all we know is the value and we have to find all the rows with that value in 
--the massive junk drawer?? 800d6025-d35e-4833-8376-ccdad717f215
Set Statistics Io ON;
--Set Statistics Io Off
SELECT COUNT(*) AS Cnt
...
/**************************************************************************************************************************************************
Chunky Data Processing (rather than doing it all in one gigantic transaction - process the data in small digestable chunks)
For Updates or Deletes - Usually only needed if you have more than 1M rows in the table and you are going to be updating or 
deleting a large percentage of them - esp. for tables with a LOT of data.
**************************************************************************************************************************************************/
